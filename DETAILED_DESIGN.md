...
- **`class BrowserManager`**
  ...
  - **Key Methods:**
    ...
    - `_send_organic_prompt_impl(platform, text)`: Brings the tab to focus. Uses JS/DOM events to clear the input box. For text > 500 chars, it uses clipboard pasting (`pyperclip`); otherwise, it simulates keystrokes (`typing_delay_ms`). Dispatches raw `input` events to bypass React state guards.
