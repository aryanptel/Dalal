...
class ContextCompressor:
    ...
    def build_context(self, chat_history: List[Dict[str, Any]], current_query: str) -> List[Dict[str, Any]]:
        """
        Selects a subset of messages from chat_history that fit the token budget,
        prioritizing recent messages, structurally central chunks (TextRank), and
        ...
        if budget <= 0:
            logging.warning("ContextCompressor: Token budget too small. Only returning recent messages.")
            return recent_messages.copy()

        # Step 2 - Chunk older messages
        older_chunks = []
        chunk_mappings = []  # Maps chunk index to original message indices
        ...
        if current_query.strip():
            ...
            avgdl = np.mean(doc_lengths) if len(doc_lengths) > 0 else 1.0
        ...
        tr_scaled = min_max_scale(textrank_scores)
        ...
        for idx in sorted_indices:
            ...
            if chunk_tokens <= budget:
                ...
                # Fallback: if chunk is too large, attempt to select individual messages within chunk
